...
class ChatRepository @Inject constructor(
    ...
    suspend fun sendMessage(auth: String, message: Message, chatRoomId: String?): Message? {
        ...
        if (chatRoomId == null) {
            ...
            val messagesRef = database.getReference(PATH_CHAT_ROOMS).child(chatRoomUid).child(PATH_MESSAGES)
            messagesRef.push().setValue(message)
            return message
        } else {
            val messagesRef = database.getReference(PATH_CHAT_ROOMS).child(chatRoomId).child(PATH_MESSAGES)
            messagesRef.push().setValue(message)
            return message
        }
    }
